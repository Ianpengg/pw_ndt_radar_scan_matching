import argparse
import os
import numpy as np
import cv2
import math
from datetime import datetime
import time
import decimal
import sys

###############################################
# Resolution of the cartesian form of the radar scan in metres per pixel
cart_resolution = .17361 # m
# Cartesian visualisation size (used for both height and width)
cart_pixel_width = 1152 # pixels
# data path
path = "/media/ee904/Data_stored/radiate/data/radiate/"
# output folder name
directory = "radar_cart_0.7361_2001_timestamped"
###############################################


def get_args():
    parser = argparse.ArgumentParser(description='Process some integers.')
    parser.add_argument('--folder_name', type=str, default="city_3_0", help='The name of the folder to be processed.')
    parser.add_argument('--cart_resolution', type=float, default=.17361, help='The resolution of the cartesian form of the radar scan in metres per pixel.')
    parser.add_argument('--cart_pixel_width', type=int, default=1152, help='The cartesian visualisation size (used for both height and width).')
    parser.add_argument('--path', type=str, default="/media/ee904/Data_stored/radiate/data/radiate/", help='The path to the data.')
    parser.add_argument('--directory', type=str, default="radar_cart_0.7361_2001_timestamped", help='The output folder name.')
    return parser.parse_args()
    
radar_init_t = 0
radar_end_t = 0
def load_timestamp(timestamp_path):
        """load all timestamps from a sensor

        :param timestamp_path: path to text file with all timestamps
        :type timestamp_path: string
        :return: list of all timestamps
        :rtype: dict
        """
        with open(timestamp_path, "r") as file:
            lines = file.readlines()
            timestamps = {'frame': [], 'time': []}
            for line in lines:
                words = line.split()
                timestamps['frame'].append(int(words[1]))
                timestamps['time'].append((words[3]))
        return timestamps

def UnixTimeToSec(unix_timestamp):

    unix_timestamp = np.ceil(unix_timestamp * 1e6)

    time = datetime.fromtimestamp(unix_timestamp / 1000000)
    s = unix_timestamp % 1000000
    sec_timestamp = time.hour*3600 + time.minute*60 + time.second + (float(s)/1000000)
    return sec_timestamp


def main():
    args = get_args()
    # Set the parameters using the values from the command-line arguments
    cart_resolution = args.cart_resolution
    cart_pixel_width = args.cart_pixel_width
    path = args.path
    directory = args.directory

    specific_foldername = args.folder_name
    
    
    dir_path = os.path.join(path, directory)
    if not os.path.exists(dir_path):
        os.mkdir(dir_path)
    print("Directory '%s' created" %directory)

    for foldername in os.listdir(path):
        print(foldername)

        if specific_foldername != None:
          if not foldername == specific_foldername:
            continue
        print("Processing")
        dir_path = os.path.join(path+'/'+directory, foldername)
        if not os.path.exists(dir_path):
            os.mkdir(dir_path)
        print("Directory '%s' created" %foldername)

        timestamps_path = path+'/'+foldername+'/Navtech_Polar.txt'

        if not os.path.isfile(timestamps_path):
            raise IOError("Could not find timestamps file")

        interpolate_crossover = True

        title = "Radar Visualisation Example"

        radar_timestamps = load_timestamp(timestamps_path)
        str_format = '{:06d}'
        time2id_map = {}
        for i in range(len(radar_timestamps['time'])):
            time2id_map[radar_timestamps['time'][i]] = str_format.format(radar_timestamps['frame'][i])
        
        #print(time2id_map)
        
        radar_init_t = UnixTimeToSec(np.float64(radar_timestamps['time'][0]))
        radar_end_t = UnixTimeToSec(np.float64(radar_timestamps['time'][np.size(radar_timestamps['time'])-1]))
        radar_t_list = np.array([], dtype='str')
        #print(radar_init_t, radar_end_t)
        for radar_timestamp in radar_timestamps['time']:
            #print(type(radar_init_t))
            curr_radar_t = UnixTimeToSec(np.float64(radar_timestamp)) - radar_init_t
            curr_radar_t_str = str("%010.5f"%curr_radar_t)
            #print(curr_radar_t, curr_radar_t_str)
            radar_t_list = np.append(radar_t_list, [curr_radar_t_str], axis=0)
            frame = time2id_map[radar_timestamp]
            filename = path + '/' + foldername + "/Navtech_Polar/" + str(frame) + '.png'

            if not os.path.isfile(filename):
                print("Could not find radar example: {}".format(filename))

            ##########################################################################################
            ##########################################################################################
            radar_resolution = np.array([0.175], np.float32)
            

            azimuths = np.arange(400) * (math.pi/180)*0.9
            
            raw_example_data = cv2.imread(filename, cv2.IMREAD_GRAYSCALE)
            #azimuths = (raw_example_data[:, 8:10].copy().view(np.uint16) / float(encoder_size) * 2 * np.pi).astype(np.float32)
            raw_example_data = raw_example_data.T
            fft_data = raw_example_data[:, :].astype(np.float32)[:, :, np.newaxis] / 255.
            
            ##########################################################################################
            if (cart_pixel_width % 2) == 0:
                cart_min_range = (cart_pixel_width / 2 - 0.5) * cart_resolution
            else:
                cart_min_range = cart_pixel_width // 2 * cart_resolution
            coords = np.linspace(-cart_min_range, cart_min_range, cart_pixel_width, dtype=np.float32)
            Y, X = np.meshgrid(coords, -coords)
            sample_range = np.sqrt(Y * Y + X * X)
            sample_angle = np.arctan2(Y, X)
            sample_angle += (sample_angle < 0).astype(np.float32) * 2. * np.pi

            # Interpolate Radar Data Coordinates
            azimuth_step = azimuths[1] - azimuths[0]

            sample_u = (sample_range - radar_resolution / 2) / radar_resolution
            sample_v = (sample_angle - azimuths[0]) / azimuth_step

            # We clip the sample points to the minimum sensor reading range so that we
            # do not have undefined results in the centre of the image. In practice
            # this region is simply undefined.
            sample_u[sample_u < 0] = 0

            if interpolate_crossover:
                fft_data = np.concatenate((fft_data[-1:], fft_data, fft_data[:1]), 0)
                sample_v = sample_v + 1

            polar_to_cart_warp = np.stack((sample_u, sample_v), -1)
            cart_img = np.expand_dims(cv2.remap(fft_data, polar_to_cart_warp, None, cv2.INTER_LINEAR), -1)
            
    

            ##########################################################################################

            # Combine polar and cartesian for visualisation
            # The raw polar data is resized to the height of the cartesian representation
            downsample_rate = 4
            fft_data_vis = fft_data[:, ::downsample_rate]
            resize_factor = float(cart_img.shape[0]) / float(fft_data_vis.shape[0])
            fft_data_vis = cv2.resize(fft_data_vis, (0, 0), None, resize_factor, resize_factor)
            vis = cv2.hconcat((fft_data_vis, fft_data_vis[:, :10] * 0 + 1, cart_img))
            cv2.namedWindow(title, cv2.WINDOW_NORMAL)
            cv2.imshow(title, vis * 2.)  # The data is doubled to improve visualisation

            cart_img = cart_img * 255.0

            cv2.imwrite(path + "/" + directory + "/" + foldername + '/' + curr_radar_t_str + '.png', cart_img) # write
            np.savetxt(path + "/" + directory + "/" + foldername + '/radar_t_list', radar_t_list, delimiter=' ', fmt='%s')
            cv2.waitKey(1)

if __name__ == '__main__':
  main()
